...
public class AWSUploadService {
    ...
    public CompletableFuture<Void> multipartUploadAsync(String bucketName, String keyName, String contentType) {
        ...
        try {
            ...
            for (int i = 1; filePosition < contentLength; i++) {
                ...
                CompletableFuture<Void> future = CompletableFuture.runAsync(() -> {
                    ...
                    UploadPartRequest uploadRequest = new UploadPartRequest().withBucketName(bucketName)
                            .withKey(keyName).withUploadId(initResponse.getUploadId()).withPartNumber(index)
                            .withFileOffset(fP).withFile(file).withPartSize(pS);
            ...
            return allFutures
                    .thenApply(
                            future -> futures.stream().map(CompletableFuture<Void>::join).collect(Collectors.toList()))
                    .thenAcceptAsync(l -> {
    ...
    public File downloadFile(String bucketName, String fileName) {
        ...
        try {
            ...
            // This returns an ObjectMetadata file but you don't have to use this if you
            // don't want
    ...
    private ForkJoinPool getJaxbExecutor() {
        ...
        int parallelism = Math.min(0x7fff /* copied from ForkJoinPool.java */,
